...
import { CheckCircle, XCircle, Eye, Users, Clock, CheckSquare, X, Loader } from 'lucide-react';
  ...
  const fetchReservations = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Erreur lors de la récupération des réservations');
      }
    } catch (err: any) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg shadow p-4 border-l-4 border-yellow-500">
            ...
            <div className="mt-2">
              <p className="text-xs text-gray-500">
                {stats.pending > 0 ? `${stats.pending} à confirmer` : 'Aucune en attente'}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
            ...
            <div className="mt-2">
              <p className="text-xs text-gray-500">
                {stats.confirmed > 0 ? `${Math.round((stats.confirmed / stats.total) * 100)}% du total` : 'Aucune confirmée'}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-4 border-l-4 border-red-500">
            ...
            <div className="mt-2">
              <p className="text-xs text-gray-500">
                {stats.cancelled > 0 ? `${Math.round((stats.cancelled / stats.total) * 100)}% du total` : 'Aucune annulée'}
              </p>
            </div>
          </div>
        </div>
                ...
                {reservations.map((reservation) => (
                  <tr key={reservation.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-sm text-gray-500">
                        {reservation.service.duration} min - {reservation.service.price}€
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
