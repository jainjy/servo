...
const TYPES_DOCUMENTS = {
  ASSURANCE: { label: "Assurance", color: "bg-blue-100 text-blue-800", icon: Shield },
  DIPLOME: { label: "Diplôme", color: "bg-green-100 text-green-800", icon: Award },
  CERTIFICATION: { label: "Certification", color: "bg-purple-100 text-purple-800", icon: FileCheck },
  CONTRAT: { label: "Contrat", color: "bg-orange-100 text-orange-800", icon: FileText },
  CGV: { label: "CGV/CGU", color: "bg-red-100 text-red-800", icon: Shield },
  ARCHIVE: { label: "Archive", color: "bg-gray-100 text-gray-800", icon: Archive }
};
...
const contratsTypes = [
  {
    ...
    description: "Contrat standard pour la vente exclusive d'un bien immobilier",
...
Le présent mandat est conclu pour une durée de [DUREE] mois...`,
    variables: ["PROPRIETAIRE", "ADRESSE", "AGENCE", "NOM_AGENT", "DUREE", "PRIX", "COMMISSION"],
    utilise: 45,
...
Le présent contrat a pour objet la location du logement...`,
    variables: ["BAILLEUR", "ADRESSE", "LOCATAIRE", "ADRESSE_LOCATAIRE", "LOYER", "CAUTION", "DURÉE"],
    utilise: 32,
...
Par le présent compromis, les parties conviennent de la vente...`,
    variables: ["VENDEUR", "ADRESSE", "ACHETEUR", "ADRESSE_ACHETEUR", "PRIX", "DATE_SIGNATURE", "NOTAIRE"],
    utilise: 28,
...
const ModalUploadDocument = ({ isOpen, onClose, onUpload }) => {
      ...
      <form onSubmit={handleUpload} className="space-y-6">
        <div>
          ...
          <Input
            ...
            placeholder="Ex: Assurance RC Pro 2024"
          />
        </div>
...
const ModalEditeurContrat = ({ isOpen, onClose, contrat, onSave }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={contrat ? "Modifier le contrat" : "Nouveau contrat type"} size="xl">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-yellow-600 mb-2">{stats.expirant}</div>
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-red-600 mb-2">{stats.expire}</div>
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-green-600 mb-2">{stats.contrats}</div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {documentsFiltres.map((document) => {
                  const TypeIcon = TYPES_DOCUMENTS[document.type]?.icon || FileText;
                  return (
                    <Card key={document.id} className="p-6 hover:shadow-lg transition-all duration-300">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg ${TYPES_DOCUMENTS[document.type]?.color}`}>
                            <TypeIcon size={20} />
                          </div>
                          <div>
                            <Badge className={STATUT_DOCUMENT[document.statut].color}>
                        ...
                        <div className="flex gap-1">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => telechargerDocument(document.url, document.nom)}
                          >
                            <Download size={16} />
                          </Button>
                          <Button
                      ...
                      <h3 className="font-semibold mb-2 line-clamp-2" style={{ color: '#0A0A0A' }}>
                        {document.nom}
                      </h3>
                      
                      <div className="space-y-2 text-sm" style={{ color: '#5A6470' }}>
                        ...
                        {document.dateExpiration && (
                          <div className="flex justify-between">
                            <span>Expire le:</span>
                            <span className={
                              document.statut === "EXPIRE" ? "text-red-600 font-semibold" :
                              document.statut === "EXPIRANT" ? "text-yellow-600 font-semibold" : ""
                            }>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {contrats.map((contrat) => (
                  <Card key={contrat.id} className="p-6 hover:shadow-lg transition-all duration-300">
                    <div className="flex items-start justify-between mb-4">
                      <Badge variant="outline">
                    ...
                    <p className="text-sm mb-4 line-clamp-2" style={{ color: '#5A6470' }}>
                      {contrat.description}
                    </p>
                    ...
                    <div className="space-y-2 text-sm" style={{ color: '#5A6470' }}>
                      <div className="flex justify-between">
                        <span>Utilisé:</span>
                        <span className="font-semibold">{contrat.utilise} fois</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Dernière modification:</span>
                        <span>{new Date(contrat.derniereModification).toLocaleDateString('fr-FR')}</span>
                      </div>
                    </div>
            ...
            <>
              ...
              <div className="space-y-4">
                {archives.map((archive) => (
                  <Card key={archive.id} className="p-6 hover:shadow-lg transition-all duration-300">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg ${TYPES_DOCUMENTS[archive.type]?.color}`}>
                          <FileText size={20} />
                        </div>
                        <div>
                    ...
                    <h3 className="font-semibold mb-3" style={{ color: '#0A0A0A' }}>
                      {archive.nom}
                    </h3>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm" style={{ color: '#5A6470' }}>
                      <div>
                      ...
                      <div>
                        <div className="font-medium mb-1">Date de signature:</div>
                        <div>{new Date(archive.dateSignature).toLocaleDateString('fr-FR')}</div>
                      </div>
                      <div>
