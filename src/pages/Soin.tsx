...
import { Activity, Moon, Heart, Star, Users, Clock, Award, CheckCircle, Shield, Leaf, Calendar, Search, Filter, ChevronDown, X, Sparkles, Zap } from "lucide-react";
  ...
  return (
    <div className="group relative bg-white dark:bg-card rounded-2xl shadow-lg hover:shadow-xl transition-all duration-500 overflow-hidden border border-gray-200 dark:border-border hover:border-primary-dark transform hover:-translate-y-1">
      ...
      <div className="relative h-56 overflow-hidden">
        <img
          src={service.images?.[0] || "https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80"}
          alt={service.libelle}
          className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
          onError={(e) => {
            e.currentTarget.src = "https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80";
          }}
        />
      ...
      <div className="p-6 space-y-4">
        <div className="flex justify-between items-start">
          ...
          {service.category && (
            <span className="text-xs bg-logo/10 text-logo px-2 py-1 rounded-full ml-2 flex-shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-gray-500 dark:text-muted-foreground">
                {service.specialist.specialty || "Spécialiste bien-être"}
              </div>
            </div>
          </div>
        )}
...
const Soin = () => {
  ...
  const fetchServices = async () => {
    setIsLoading(true);
    try {
      console.log('📡 [Soin] Début récupération services');
      
      // Test d'abord la route simple
      try {
        console.log('🧪 Test route /test');
        const testResponse = await api.get('/soins-bienetre/test');
      ...
      const response = await api.get('/soins-bienetre', { params });
      ...
      console.error('❌ Erreur récupération services soins:', {
  ...
  const fetchCategories = async () => {
    try {
      console.log('📡 Récupération catégories soins...');
      const response = await api.get('/soins-bienetre/categories');
      if (response.data.success) {
        setCategories(response.data.categories);
        console.log('✅ Catégories soins chargées:', response.data.categories);
      } else {
        console.warn('⚠️ Erreur chargement catégories, utilisation par défaut');
        setCategories(['Tous', 'Soins', 'Massages', 'Détente', 'Esthétique']);
      }
    } catch (error) {
      console.error('❌ Erreur récupération catégories soins:', error.message);
      setCategories(['Tous', 'Soins', 'Massages', 'Détente', 'Esthétique']);
    }
  };
  ...
  const fetchStats = async () => {
    try {
      console.log('📡 Récupération statistiques soins...');
      const response = await api.get('/soins-bienetre/stats');
      if (response.data.success) {
        setStats(response.data.stats);
        console.log('✅ Statistiques soins mises à jour');
      }
    } catch (error) {
      console.error('❌ Erreur récupération statistiques soins:', error.message);
    }
  };

  useEffect(() => {
    console.log('🔄 Soin component mounted, chargement initial...');
    fetchServices();
    fetchCategories();
    fetchStats();
  }, []);
  ...
  const tabs = categories.map(category => ({
    id: category,
    label: category === 'Tous' ? 'Tous les soins' : category,
    icon: category === 'Tous' ? <Heart className="w-5 h-5" /> : 
           category === 'Soins' ? <Moon className="w-5 h-5" /> :
           category === 'Massages' ? <Activity className="w-5 h-5" /> :
           category === 'Détente' ? <Leaf className="w-5 h-5" /> :
           <Award className="w-5 h-5" />,
    description: category === 'Tous' ? 'Tous nos soins' : 
                 `Soins de ${category.toLowerCase()}`
  }));
  ...
  const statCards = [
    {
      ...
      label: "Soins disponibles",
      description: "Services actifs",
      color: "logo"
    },
    {
      icon: Heart,
      value: stats.satisfiedClients ? `${stats.satisfiedClients}+` : "1250+",
    ...
    {
      icon: Star,
      value: stats.satisfactionRate ? `${stats.satisfactionRate}%` : "97%",
    ...
    {
      icon: Award,
      value: stats.averageRating ? `${stats.averageRating}/5` : "4.8/5",
      label: "Note moyenne",
      description: "Sur 500+ avis",
      color: "primary-dark"
    }
  ];
    ...
    <div className="font-sans text-foreground">
            ...
            <div className="flex flex-col lg:flex-row gap-4 mb-4">
              <div className="lg:w-1/3">
                <div className="relative">
                  <input
                    type="text"
                    placeholder="Rechercher un soin, un massage..."
                    value={searchTerm}
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                {services.length === 0 ? 'Aucun soin trouvé' : `${services.length} soins disponibles`}
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="flex justify-between items-center mb-8">
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mt-2">
                    {sortedServices.length} soin{sortedServices.length !== 1 ? 's' : ''} disponible{sortedServices.length !== 1 ? 's' : ''}
                ...
                {sortedServices.length > 0 ? (
                  ...
                  <div className="text-center py-12 bg-gray-50 dark:bg-gray-800/30 rounded-2xl">
                    ...
                    <h4 className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">
                      {searchTerm ? 'Aucun soin ne correspond à votre recherche' : 'Aucun soin disponible'}
                    </h4>
                    <p className="text-gray-600 dark:text-gray-400 mb-4">
                      {searchTerm 
                        ? "Essayez de modifier vos critères de recherche ou vos filtres." 
                        : "Les soins seront bientôt disponibles."}
                    </p>
                    {searchTerm && (
                      <button
                        onClick={handleClearFilters}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-12">
        <div className="bg-white dark:bg-card rounded-3xl shadow-lg p-8 border border-gray-200 dark:border-gray-700/40">
          <h3 className="text-2xl font-bold mb-6" style={{ color: '#8B4513' }}>Notre approche du bien-être</h3>
          <div className="space-y-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 dark:text-muted-foreground text-sm">Nous considérons le bien-être dans sa globalité : corps, esprit et énergie.</p>
              </div>
            </div>
            <div className="flex items-start gap-4">
              <div className="p-3 bg-logo/10 rounded-xl">
              ...
              <div>
                ...
                <p className="text-gray-600 dark:text-muted-foreground text-sm">Utilisation exclusive de produits bio, naturels et respectueux de la peau.</p>
              </div>
            </div>
            <div className="flex items-start gap-4">
              <div className="p-3 bg-logo/10 rounded-xl">
              ...
              <div>
                <h4 className="font-bold text-gray-800 dark:text-foreground mb-2">Hygiène rigoureuse</h4>
                <p className="text-gray-600 dark:text-muted-foreground text-sm">Des protocoles d'hygiène stricts pour votre sécurité et confort.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-card rounded-3xl shadow-lg p-8 border border-gray-200 dark:border-gray-700/40">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
              <span className="font-medium text-gray-700 dark:text-gray-300">Spécialistes certifiés</span>
              <CheckCircle className="w-6 h-6 text-logo" />
            </div>
            <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
              <span className="font-medium text-gray-700 dark:text-gray-300">Matériel professionnel</span>
              <CheckCircle className="w-6 h-6 text-logo" />
            </div>
            <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
              <span className="font-medium text-gray-700 dark:text-gray-300">Ambiance relaxante</span>
              <CheckCircle className="w-6 h-6 text-logo" />
            </div>
            <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
              <span className="font-medium text-gray-700 dark:text-gray-300">Suivi personnalisé</span>
              <CheckCircle className="w-6 h-6 text-logo" />
            </div>
            <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
              <span className="font-medium text-gray-700 dark:text-gray-300">Conseils après-soin</span>
              <CheckCircle className="w-6 h-6 text-logo" />
            </div>
          </div>
        </div>
      ...
      <SlideIn direction="up" delay={400}>
        <div className="mt-12 bg-gradient-to-r from-logo/10 to-primary-dark/10 rounded-2xl p-8 border border-logo/20">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h3 className="text-xl font-bold" style={{ color: '#8B4513' }}>Forfaits Découverte</h3>
              <p className="text-gray-600 dark:text-gray-400">Profitez de nos formules avantageuses</p>
            </div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white dark:bg-card rounded-2xl p-6 border border-separator dark:border-border">
              <div className="font-bold text-lg mb-2">Forfait Détente</div>
              <div className="text-2xl font-bold text-logo mb-2">150€<span className="text-sm text-gray-500">/3 soins</span></div>
            ...
            <div className="bg-white dark:bg-card rounded-2xl p-6 border border-separator dark:border-border relative">
              ...
              <div className="font-bold text-lg mb-2">Forfait Bien-être</div>
              <div className="text-2xl font-bold text-logo mb-2">280€<span className="text-sm text-gray-500">/5 soins</span></div>
              <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                <li className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded-full bg-green-500 text-white flex items-center justify-center flex-shrink-0 text-xs">
                    ✓
                  </div>
                  <span>Soins variés au choix</span>
                </li>
                <li className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded-full bg-green-500 text-white flex items-center justify-center flex-shrink-0 text-xs">
                    ✓
                  </div>
                  <span>Crème hydratante offerte</span>
                </li>
                <li className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded-full bg-green-500 text-white flex items-center justify-center flex-shrink-0 text-xs">
                    ✓
            ...
            <div className="bg-white dark:bg-card rounded-2xl p-6 border border-separator dark:border-border">
              ...
              <div className="text-2xl font-bold text-logo mb-2">500€<span className="text-sm text-gray-500">/10 soins</span></div>
              <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                <li className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded-full bg-green-500 text-white flex items-center justify-center flex-shrink-0 text-xs">
                    ✓
                  </div>
                  <span>Tous nos soins inclus</span>
                </li>
                <li className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded-full bg-green-500 text-white flex items-center justify-center flex-shrink-0 text-xs">
                    ✓
                ...
                <li className="flex items-center gap-2">
                  ...
                  <span>Cadeau bien-être offert</span>
                </li>
              </ul>
            </div>
          </div>
  ...
  return [
    {
      ...
      tags: ["visage", "anti-âge", "peau", "naturel"],
      popular: true
    },
    {
      id: 2,
      ...
      tags: ["massage", "suédois", "détente", "muscles"],
      popular: true
    },
    {
      id: 3,
      libelle: "Séance de Relaxation Profonde",
      description: "Séance complète de relaxation incluant sophrologie, respiration consciente et visualisation.",
      price: 70,
      duration: 60,
      durationFormatted: "1h",
      ...
      benefits: "Stress réduit, sommeil amélioré, équilibre intérieur",
      specialist: {
        name: "Marie Dubois",
      ...
      features: [
        ...
        "Guidance vocale douce",
        "Ambiance sensorielle"
      ],
      tags: ["relaxation", "sophrologie", "stress", "bien-être"],
      popular: false
    }
  ];
}
