...
export default function TopographicPage() {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      const height = canvas.parentElement?.clientHeight || window.innerHeight * 2
    ...
    const updateNoiseGrid = (t: number, width: number, height: number) => {
      ...
      for (let i = 0; i <= cols; i++) {
        ...
        for (let j = 0; j <= rows; j++) {
          ...
          let value = 0
          value += Math.sin(x * scale1 + t * 0.3) * Math.cos(y * scale1 + t * 0.24) * 0.4
          value += Math.sin(x * scale2 - t * 0.16) * Math.cos(y * scale2 + t * 0.2) * 0.25
          value += Math.sin((x + y) * scale3 + t * 0.12) * 0.2
          value += Math.sin(x * scale4 * 2 + t * 0.1) * Math.sin(y * scale4 * 2 + t * 0.14) * 0.15
    ...
    const noise = (x: number, y: number) => {
      ...
      if (mouseRef.current.active) {
        ...
        if (distance < maxRadius) {
          ...
          const intensity = 1000 - t * t * (3 - 2 * t) // smoothstep easing
    ...
    const animate = (currentTime: number) => {
      ...
      ctx.fillStyle = "#252c1d"
      ctx.fillRect(0, 0, width, height)

      ctx.imageSmoothingEnabled = true
      ctx.imageSmoothingQuality = "high"

      const levels = 8 // Reduced from 25 to 12 lines
      const resolution = 8
