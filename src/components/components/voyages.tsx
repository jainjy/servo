...
const VoyagesAeriens: React.FC = () => {
    ...
    const volsDemo: Vol[] = [
        {
            ...
            rating: 4.5,
            reviewCount: 1247,
            services: ['wifi', 'repas', 'divertissement', 'bagage-cabine'],
            aircraft: 'Boeing 777',
            baggage: {
                cabine: '1 bagage + 1 accessoire',
                soute: '1 bagage inclus'
            }
        },
        {
            id: '2',
    ...
    const compagnies = ['toutes', 'Air France', 'Emirates', 'Delta Air Lines', 'Qatar Airways', 'British Airways'];
        ...
        return (
            <div className="flex items-center space-x-1">
                {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                        key={star}
                        className={`w-3 h-3 ${star <= Math.floor(note)
        ...
        return (
            <div className="flex flex-wrap gap-1">
                {services.map(serviceId => {
                    ...
                    const IconComponent = service?.icon || CheckCircle;
                    return (
                        <div key={serviceId} className="flex items-center p-1 bg-blue-50 rounded-lg">
                            <IconComponent className="w-3 h-3 text-blue-600 mr-1" />
                            <span className="text-xs text-blue-700">{service?.label}</span>
                        </div>
                    );
                })}
            </div>
    ...
    const getClasseColor = (classe: string) => {
        const colors: { [key: string]: string } = {
            economique: 'text-green-600 bg-green-50',
            affaires: 'text-blue-600 bg-blue-50',
            premiere: 'text-purple-600 bg-purple-50'
        };
        return colors[classe] || 'text-gray-600 bg-gray-50';
    };
        ...
        return (
            <div className="min-h-screen mt-16 py-8 flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-600">Recherche des vols en cours...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen mt-16 py-8 bg-gray-50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
                    ...
                    {/* Formulaire de recherche détaillé */}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    type="date"
                                    value={recherche.dateAller}
                                    onChange={(e) => setRecherche({...recherche, dateAller: e.target.value})}
                                    className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                />
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <select
                                    value={recherche.classe}
                                    onChange={(e) => setRecherche({...recherche, classe: e.target.value})}
                                    className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 appearance-none"
                                >
                                    <option value="economique">1 Passager - Économique</option>
                                    <option value="affaires">1 Passager - Affaires</option>
                                    <option value="premiere">1 Passager - Première</option>
                                </select>
                                ...
                                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-start justify-between mb-4">
                                            <div>
                                                <div className="flex items-center space-x-3 mb-2">
                                                    ...
                                                    <div>
                                                        <h3 className="font-semibold text-gray-900">{vol.compagnie}</h3>
                                                        <p className="text-sm text-gray-500">{vol.numeroVol}</p>
                                                    </div>
                                                </div>
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getClasseColor(vol.classe)}`}>
                                        ...
                                        <div className="flex items-center justify-between mb-4">
                                            <div className="text-center">
                                                <div className="text-lg font-semibold text-gray-900">{vol.depart.heure}</div>
                                                <div className="text-sm text-gray-600">{vol.depart.code}</div>
                                                <div className="text-xs text-gray-500">{vol.depart.ville}</div>
                                            </div>
                                            ...
                                            <div className="text-center">
                                                <div className="text-lg font-semibold text-gray-900">{vol.arrivee.heure}</div>
                                                <div className="text-sm text-gray-600">{vol.arrivee.code}</div>
                                                <div className="text-xs text-gray-500">{vol.arrivee.ville}</div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex items-center justify-between text-sm">
                                                <div className="flex items-center space-x-4">
                                                    <span className="text-gray-600">{vol.aircraft}</span>
                                                    <div className="flex items-center space-x-1">
                                                        {renderStars(vol.rating)}
                                                        <span className="text-gray-600">({vol.reviewCount})</span>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div className="lg:w-48 flex lg:flex-col gap-3">
                                        ...
                                        <div className="flex space-x-2">
                                            ...
                                            >
                                                <Heart className={`w-4 h-4 ${isFavori(vol.id) ? 'fill-current' : ''}`} />
                                            </button>
                                            <button 
                                                onClick={() => partagerVol(vol)}
            ...
            {/* Modal de Réservation - Format PC écran */}
